...
void euler_step(State& state, const Constants& constants, const ExternalForces& forces, double dt) {
    // Вычисляем производные в текущий момент времени
    State derivatives = compute_derivatives(state, constants, forces);
    
    // Простое обновление методом Эйлера: y(t + dt) = y(t) + dy/dt * dt
    state.position += derivatives.position * dt;
    state.momentum += derivatives.momentum * dt;
    state.angular_momentum += derivatives.angular_momentum * dt;
    
    // Обновление кватерниона
    state.quaternion.x += derivatives.quaternion.x * dt;
    state.quaternion.y += derivatives.quaternion.y * dt;
    state.quaternion.z += derivatives.quaternion.z * dt;
    state.quaternion.w += derivatives.quaternion.w * dt;
    state.quaternion = glm::normalize(state.quaternion);
    
    // Обновление потенциальной энергии плавучести
    state.integrated_potential_energy_buoyancy += derivatives.integrated_potential_energy_buoyancy * dt;
}
void print_energy(const State& state_arg, const Constants& consts_arg, const ExternalForces& forces_arg) {
...
void update(int value) {
    euler_step(current_state, sim_constants, ext_forces, dt);
    simulation_time += dt;
