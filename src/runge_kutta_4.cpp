...
void runge_kutta_4(State& state_ref, const Constants& consts_ref, const ExternalForces& forces_ref, double time_step) {
    State k1 = compute_derivatives(state_ref, consts_ref, forces_ref);
    State k2 = compute_derivatives(state_ref + k1 * (0.5 * time_step), consts_ref, forces_ref);
    State k3 = compute_derivatives(state_ref + k2 * (0.5 * time_step), consts_ref, forces_ref);
    State k4 = compute_derivatives(state_ref + k3 * time_step, consts_ref, forces_ref); // k3 * time_step, не k3เฉยๆ

    state_ref.position += (k1.position + k2.position * 2.0 + k3.position * 2.0 + k4.position) * (time_step / 6.0);
    state_ref.momentum += (k1.momentum + k2.momentum * 2.0 + k3.momentum * 2.0 + k4.momentum) * (time_step / 6.0);
    state_ref.angular_momentum += (k1.angular_momentum + k2.angular_momentum * 2.0 + k3.angular_momentum * 2.0 + k4.angular_momentum) * (time_step / 6.0);
    
    // Обновление кватерниона (сложение производных, затем нормализация)
    state_ref.quaternion.x += (k1.quaternion.x + 2.0 * k2.quaternion.x + 2.0 * k3.quaternion.x + k4.quaternion.x) * (time_step / 6.0);
    state_ref.quaternion.y += (k1.quaternion.y + 2.0 * k2.quaternion.y + 2.0 * k3.quaternion.y + k4.quaternion.y) * (time_step / 6.0);
    state_ref.quaternion.z += (k1.quaternion.z + 2.0 * k2.quaternion.z + 2.0 * k3.quaternion.z + k4.quaternion.z) * (time_step / 6.0);
    state_ref.quaternion.w += (k1.quaternion.w + 2.0 * k2.quaternion.w + 2.0 * k3.quaternion.w + k4.quaternion.w) * (time_step / 6.0);
    state_ref.quaternion = glm::normalize(state_ref.quaternion);

    // Обновление интегрированной потенциальной энергии плавучести
    state_ref.integrated_potential_energy_buoyancy += 
        (k1.integrated_potential_energy_buoyancy + 
         2.0 * k2.integrated_potential_energy_buoyancy + 
         2.0 * k3.integrated_potential_energy_buoyancy + 
         k4.integrated_potential_energy_buoyancy) * (time_step / 6.0);
}
...
void print_energy(const State& state_arg, const Constants& consts_arg, const ExternalForces& forces_arg) {
    ...
    std::cout << "Time: " << simulation_time << std::fixed << std::setprecision(2)
              ...
              << ", PE_buoy_integ: " << actual_buoyancy_potential_energy 
...
void update(int value) {
    runge_kutta_4(current_state, sim_constants, ext_forces, dt);
    simulation_time += dt;
...
int main(int argc, char** argv) {
    ...
    current_state.integrated_potential_energy_buoyancy = 0.0; 

    // Инициализация констант тела
    sim_constants.mass = 50.0; 
